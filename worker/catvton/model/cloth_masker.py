...
from worker.catvton.model.SCHP import SCHP
...
class AutoMasker:
    def __init__(
        ...
        self.use_densepose = True
        try:
            from worker.catvton.model.DensePose import DensePose  # noqa: WPS433
    ...
    def process_densepose(self, image_or_path):
        if self.use_densepose and self.densepose_processor is not None:
            return self.densepose_processor(image_or_path, resize=1024)
        schp_atr = np.array(self.schp_processor_atr(image_or_path))
        schp_lip = np.array(self.schp_processor_lip(image_or_path))
        return Image.fromarray(self._schp_to_densepose(schp_atr, schp_lip), mode="L")

    @staticmethod
    def _schp_to_densepose(schp_atr: np.ndarray, schp_lip: np.ndarray) -> np.ndarray:
        """Approximate DensePose labels from SCHP parsing when Detectron2 is unavailable."""
        dense = np.zeros_like(schp_atr, dtype=np.uint8)
        torso = (
            part_mask_of(["Upper-clothes", "Dress", "Coat", "Jumpsuits"], schp_lip, LIP_MAPPING)
            | part_mask_of(["Upper-clothes", "Dress"], schp_atr, ATR_MAPPING)
        )
        dense[torso > 0] = 1
        dense[(schp_atr == ATR_MAPPING["Left-arm"]) | (schp_lip == LIP_MAPPING["Left-arm"])] = 17
        dense[(schp_atr == ATR_MAPPING["Right-arm"]) | (schp_lip == LIP_MAPPING["Right-arm"])] = 16
        dense[(schp_atr == ATR_MAPPING["Left-leg"]) | (schp_lip == LIP_MAPPING["Left-leg"])] = 12
    ...
    def preprocess_image(self, image_or_path):
        ...
        if self.use_densepose and self.densepose_processor is not None:
            ...
            densepose = Image.fromarray(
                self._schp_to_densepose(np.array(schp_atr), np.array(schp_lip)),
                mode="L",
            )
        return {
            "densepose": densepose,
